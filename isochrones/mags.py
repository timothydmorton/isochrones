...
def interp_mag(pars, index_order, model_grid,
    ...
    n_bands = len(bc_cols)
    mags = np.empty(n_bands, dtype=float64)
...
def interp_mags(pars, index_order, model_grid,
    ...
    Teffs = np.empty(n_values, dtype=float64)
    loggs = np.empty(n_values, dtype=float64)
    fehs = np.empty(n_values, dtype=float64)
    mags = np.empty((n_values, n_bands), dtype=float64)
